...
bar_cat()
{
   ...
   # Width of the bar (in ten characters).  The default is 60 characters.
   if test -z "${BAR_WIDTH}" && test -n "${COLUMNS}"
   then
       BAR_WIDTH=${COLUMNS}
   fi
   ...
   then
       ...
       ${BAR_ECHO} "${BAR_E_C1}" "
${bar_perc}${BAR_L}${bar_graph}${BAR_R}
${BAR_E_C2}" 1>&2
       bar_shown=1
   fi
       ...
       do
           ...
           (
               ...
               do
                   ...
                   if test "x${BAR_PERC}" = x1
                   then
                       bar_perc="  "`expr ${bar_pos1} / ${bar_bph}`"% " || bar_perc=0
                   ...
                   do
                       bar_graph="${bar_graph}${BAR_C1}"
                       ${BAR_ECHO} "${BAR_E_C1}" "
${bar_perc}${bar_graph}${BAR_E_C2}" 1>&2
                       bar_cur_char=`expr ${bar_cur_char} + 1`
                   done
               done
           ) | eval "${BAR_CMD}"
...
${BAR_E_C2}" 1>&2
           ...
           ${BAR_ECHO} "${BAR_E_C1}" "
${bar_perc}${BAR_L}${bar_graph}${BAR_R}${BAR_E_C2}" 1>&2
           ${BAR_E_NL} 1>&2
       fi
   fi
}
...
do
   case "$1" in
       ...
       --) shift
           break
       ;;
       -*) echo "$0: Error: Unrecogniszed option: $1" 1>&2
           exit 1
       ;;
       *)
           break
   ...
   case "${BAR_ARG}" in
       -h) echo 'Usage: bar [-n] [-p] [-o FILE] [-c CMD] [-b SIZE] [-w WIDTH]'
           echo '           [-0/1/[/] CHAR] [-e EXT] [-d DIR] [Files]'
           echo '       bar -D'
           echo '       bar -D-'
           echo 'Options:'
           ...
           echo '     -w WIDTH   width in characters        (default: terminal width-3 or 60)'
           ...
           echo '     -n         clear bar after termination'
           echo '     -p         hide percentage'
           echo '     -D         try to dump the bar_cat() shell function, then exit'
           echo '     -D-        same as -D, but dump the function body only'
           echo '     --         last option: only file names follow'
           exit 0
       ;;
       -n) BAR_CLEAR=1
       ;;
       -p) BAR_PERC=0
       ;;
       -D) awk '{sub(/ *#.*$/,"")} ; /^bar_cat/ {x=1} ; /./ {if(x)print} ; /^}/ {x=0}' "$0"
           exit 0
       ;;
       -D-) awk '{sub(/ *#.*$/,"")} ; /^}/ {x=0} ; /./ {if(x)print} ; /^{/ {x=1}' "$0"
           exit 0
       ;;
       -o) exec 1>"${BAR_OPT}"
       ;;
